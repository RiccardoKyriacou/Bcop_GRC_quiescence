...
all_expr_confident <- TPM_genes %>%
  filter(
    ...
    Gene       %in% confident_genes
  ) %>%
  mutate(log2_TPM = log2(TPM + 1))   # remove this line if log2_TPM already exists in TPM_genes

# Step 2: Build wide matrix — one row per gene, one column per sample
heat_df <- all_expr_confident %>%
  select(Gene, Sample, log2_TPM) %>%
  group_by(Gene, Sample) %>%
  summarise(log2_TPM = mean(log2_TPM, na.rm = TRUE), .groups = "drop") %>%
...
# Step 4: Build long format for ggplot — sourced from the full expression table
df_heat_plot <- all_expr_confident %>%
  select(Gene, Sample, log2_TPM) %>%
  filter(Sample %in% col_order) %>%
  mutate(
...
blast_df <- read_tsv("Revisions\\03_expressed_GRC_genes\\GRC_BLAST_table.tsv") %>%
  mutate(
